...
CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -x -ldflags "-s -w" -o ./bin/kit-scaffold.palm.grpc.api.srv

# 重命名可执行文件
if [ -f "./bin/kit-scaffold.palm.grpc.api" ];then
  mv ./bin/kit-scaffold.palm.grpc.api ./bin/kit-scaffold.palm.grpc.api.back
fi
...
upx -9 -v -o ./bin/kit-scaffold.palm.grpc.api ./bin/kit-scaffold.palm.grpc.api.srv

# 删除服务编译文件
if [ -f "./bin/kit-scaffold.palm.grpc.api.srv" ];then
  rm -f ./bin/kit-scaffold.palm.grpc.api.srv
fi

# 删除备份可执行文件
if [ -f "./bin/kit-scaffold.palm.grpc.api.back" ];then
  rm -f ./bin/kit-scaffold.palm.grpc.api.back
fi