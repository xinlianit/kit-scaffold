...
func CancelInterceptor(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
	// 处理请求前
	log.Printf("请求开始时间: %v", util.TimeUtil().GetCurrentDateTime(constant.DefaultTimeLayout))
	...
	// 处理请求后
	log.Printf("请求完成时间: %v", util.TimeUtil().GetCurrentDateTime(constant.DefaultTimeLayout))
	return resp, err
}
